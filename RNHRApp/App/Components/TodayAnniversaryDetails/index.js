...
import { IMG_WORKANNIVERSARY } from "../../Assets/images";
...
class TodayAnniversaryDetails extends Component {
  constructor(props) {
    super(props);
    this.state = {};
  ...
  showWishTemplate(wishType, employee) {
    const { showWishTemplateView } = this.props;
    showWishTemplateView(wishType, employee);
  }
  ...
  sendWish(employee, option) {
    ...
    // console.log('Send wish for employee : ', employee);
    // console.log('Wish option ', option);
    sendWishMessage(employee, option);
  }

  rendertodayAnniversary = ({ item }) => {
        ...
        >
          ...
          <CollapseBody>
            <SwitchComponent
              ...
              showWishTemplate={this.showWishTemplate}
            />
          </CollapseBody>
        </Collapse>
      </View>
      ...
      >
        <View style={styles.headingContainer}>
          <View style={styles.imgView}>
            <Image style={styles.imgPosition} source={IMG_WORKANNIVERSARY} />
            <Text style={styles.textHeader}>
              {getText(I18N_CONSTANTS.WISHES.ANNIVERSARY_HEADER)}
            </Text>
          </View>
        </View>
        <ScrollView
        ...
        >
          ...
          />
          {anniversaryArray.length > 0 ? (
            <FlatList
              extraData={this.state}
              data={anniversaryArray}
              renderItem={this.rendertodayAnniversary}
...
TodayAnniversaryDetails.propTypes = {
  anniversaryArray: PropTypes.objectOf(PropTypes.object).isRequired,
  sendWishMessage: PropTypes.objectOf(PropTypes.object).isRequired
};

export default TodayAnniversaryDetails;
